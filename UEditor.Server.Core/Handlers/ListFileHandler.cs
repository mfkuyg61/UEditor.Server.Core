...
namespace UEditor.Server.Core.Handlers
{
    ...
    public class ListFileManager : Handler
    {
        ...
        public ListFileManager(HttpContext context, UEditorService option, string pathToList, string[] searchExtensions)
            : base(context, option)
        {
        ...
        public override void Process()
        {
            try
            {
                ...
                Size = String.IsNullOrEmpty(Request.Query["size"]) ? Option.GetInt("imageManagerListSize") : Convert.ToInt32(Request.Query["size"]);
            }
            ...
            try
            {
                ...
                var localPath = Path.Combine(Option.Config.WebRootPath, PathToList);
